...
class TXCosmoDC2Mock(PipelineStage):
    ...
    def setup_metacal_output(self, metacal_file, target_size):
        ...
        cols = (
            ...
            + metacal_variants("g1", "g2", "T", "s2n", "T_err")
            + band_variants("riz", "mag", "mag_err", shear_catalog_type="metacal")
            + ["weight"]
        )
        ...
        for col in metacal_variants("flags"):
            group.create_dataset(col, (target_size,), maxshape=(target_size,), dtype="i8")

        return cols + ["id", "flags"]
    ...
    def make_mock_metadetect(self, data, photo):
        ...
        output = {
            ...
            "ns/true_g1": g1,
            "ns/true_g2": g2,
            "ns/redshift_true": photo["redshift_true"],
            ...
            "ns/mag_err_r": photo["mag_err_r"],
            "ns/mag_err_i": photo["mag_err_i"],
            "ns/mag_err_z": photo["mag_err_z"],
            "ns/mag_r": photo["mag_r"],
            "ns/mag_i": photo["mag_i"],
            "ns/mag_z": photo["mag_z"],
            ...
            "1p/mag_err_r": photo["mag_err_r"],
            "2p/mag_err_r": photo["mag_err_r"],
            "1m/mag_err_r": photo["mag_err_r"],
            "2m/mag_err_r": photo["mag_err_r"],
            "1p/mag_err_i": photo["mag_err_i"],
            "2p/mag_err_i": photo["mag_err_i"],
            "1m/mag_err_i": photo["mag_err_i"],
            "2m/mag_err_i": photo["mag_err_i"],
            "1p/mag_err_z": photo["mag_err_z"],
            "2p/mag_err_z": photo["mag_err_z"],
            "1m/mag_err_z": photo["mag_err_z"],
            "2m/mag_err_z": photo["mag_err_z"],
            ...
            "2p/psf_g2": zero,
            "2m/psf_g2": zero,
            "ns/psf_T_mean": np.repeat(psf_T, nobj),
            "1p/psf_T_mean": np.repeat(psf_T, nobj),
            "1m/psf_T_mean": np.repeat(psf_T, nobj),
            "2p/psf_T_mean": np.repeat(psf_T, nobj),
            "2m/psf_T_mean": np.repeat(psf_T, nobj),
            # Everything that gets this far should be used, so flag=0
            "ns/flags": np.zeros(nobj, dtype=np.int32),
    ...
    def make_mock_metacal(self, data, photo):
        ...
        output = {
            ...
            # g1
            "g1": e1 * R,
            "g1_1p": (e1 + delta_gamma) * R,
            "g1_1m": (e1 - delta_gamma) * R,
            "g1_2p": e1 * R,
            "g1_2m": e1 * R,
            # g2
            "g2": e2 * R,
            "g2_1p": e2 * R,
            "g2_1m": e2 * R,
            "g2_2p": (e2 + delta_gamma) * R,
            "g2_2m": (e2 - delta_gamma) * R,
            # T
            "T": size_T,
            "T_1p": size_T + R_size * delta_gamma,
            "T_1m": size_T - R_size * delta_gamma,
            "T_2p": size_T + R_size * delta_gamma,
            "T_2m": size_T - R_size * delta_gamma,
            # Terr
            "T_err": zero,
            "T_err_1p": zero,
            "T_err_1m": zero,
            ...
            "mag_i": photo["mag_i"],
            "mag_z": photo["mag_z"],
            "mag_err_r": photo["mag_err_r"],
            "mag_err_i": photo["mag_err_i"],
            "mag_err_z": photo["mag_err_z"],
            "mag_r_1p": photo["mag_r_1p"],
            "mag_r_2p": photo["mag_r_2p"],
            "mag_r_1m": photo["mag_r_1m"],
            "mag_r_2m": photo["mag_r_2m"],
            "mag_i_1p": photo["mag_i_1p"],
            "mag_i_2p": photo["mag_i_2p"],
            "mag_i_1m": photo["mag_i_1m"],
            "mag_i_2m": photo["mag_i_2m"],
            "mag_z_1p": photo["mag_z_1p"],
            "mag_z_2p": photo["mag_z_2p"],
            "mag_z_1m": photo["mag_z_1m"],
            "mag_z_2m": photo["mag_z_2m"],
            "mag_err_r_1p": photo["mag_err_r"],
            "mag_err_r_2p": photo["mag_err_r"],
            "mag_err_r_1m": photo["mag_err_r"],
            "mag_err_r_2m": photo["mag_err_r"],
            "mag_err_i_1p": photo["mag_err_i"],
            "mag_err_i_2p": photo["mag_err_i"],
            "mag_err_i_1m": photo["mag_err_i"],
            "mag_err_i_2m": photo["mag_err_i"],
            "mag_err_z_1p": photo["mag_err_z"],
            "mag_err_z_2p": photo["mag_err_z"],
            "mag_err_z_1m": photo["mag_err_z"],
            "mag_err_z_2m": photo["mag_err_z"],
            # Fixed PSF parameters - all round with same size
            "psf_g1": zero,
            "psf_g2": zero,
            "psf_T_mean": np.repeat(psf_T, nobj),
            # Everything that gets this far should be used, so flag=0
            "flags": np.zeros(nobj, dtype=np.int32),
            "flags_1p": np.zeros(nobj, dtype=np.int32),
            "flags_1m": np.zeros(nobj, dtype=np.int32),
            "flags_2p": np.zeros(nobj, dtype=np.int32),
            "flags_2m": np.zeros(nobj, dtype=np.int32),
            # we use weights of one for everything for metacal
            # if that ever changes we may also need to add
            # weight_1p, etc.
            "weight": np.ones(nobj),
