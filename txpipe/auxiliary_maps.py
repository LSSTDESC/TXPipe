...
class TXSourceAuxMapsBase(PipelineStage):
    name = "TXSourceAuxMapsBase"
...
class TXPSFMaps(TXSourceAuxMapsBase):
    name = "TXPSFMaps"
    dask_parallel = True
...
class TXFlagMaps(TXSourceAuxMapsBase):
    ...
    def run(self):
        ...
        for i in range(flag_exponent_max):
            maps[f"flags/flag_{2**i}"] = flag_maps[i]
        ...
        # write the output maps
        with self.open_output("flag_maps", wrapper=True) as out:
            for map_name, hsp_map in hsp_maps.items():
                out.write_map(map_name, hsp_map, metadata)
            out.file["maps"].attrs.update(metadata)


class TXDepthMaps(TXBaseMaps):
    """
    Generate depth maps from the lens photometry catalog.
    """
    ...
    def run(self):
        ...
        maps = {
            "depth/depth": depth_map_results["depth_map"],
            "depth/depth_count": depth_map_results["count_map"],
            "depth/depth_var": depth_map_results["depth_var"],
        }
        ...
        with self.open_output("depth_map", wrapper=True) as out:
            for map_name, hsp_map in hsp_maps.items():
                out.write_map(map_name, hsp_map, metadata)
            out.file["maps"].attrs.update(metadata)


class TXBrightObjectMaps(TXBaseMaps):
    """
    Generate bright-object count maps from the lens photometry catalog.
    """

    name = "TXBrightObjectMaps"
    dask_parallel = True
    inputs = [
        ("photometry_catalog", HDFFile),  # for mags etc
    ]
    ...
    def run(self):
        ...
        if block_size == 0:
            block_size = "auto"
        band = self.config.get("depth_band", "i")
        ...
        with self.open_output("bright_object_map", wrapper=True) as out:
            for map_name, hsp_map in hsp_maps.items():
                out.write_map(map_name, hsp_map, metadata)
            out.file["maps"].attrs.update(metadata)
