...
class TXSelector(PipelineStage):
    ...
    def run(self):
        ...
        shear_cols = ['f{shear_prefix}_flags', 'f{shear_prefix}psf_T_mean','mask',shear_catalog_type=self.config['shear_catalog_type']]
        shear_cols += band_variants(bands, 'f{shear_prefix}_mag', 'f{shear_prefix}_mag_err')
        if self.config['shear_catalog_type']=='metacal':
            shear_cols += metacal_variants('f{shear_prefix}_T', 'f{shear_prefix}_s2n', 'f{shear_prefix}_g1', 'f{shear_prefix}_g2')
        ...
        nbin_source = self.config['nsrc']
        nbin_lens = self.config['nlens']

        selection_biases = []
        number_density_stats = NumberDensityStats(nbin_source, nbin_lens, self.comm,shear_type=self.config['shear_catalog_type'])
        if self.config['shear_catalog_type']=='metacal':
        ...
        for (start, end, shear_data), (_, _, phot_data) in zip(iter_shear, iter_phot):
            ...
            if self.config['select_lens']:
                lens_gals = self.select_lens(phot_data)
            else:
                lens_gals = None
            # Save the tomography for this chunk 
            self.write_tomography(output_file, start, end, tomo_bin, lens_gals, R)
    ...
    def apply_classifier(self, classifier, features, shear_data):
        ...
        for v in variants:
            ...
            for f in features:
                ...
                if len(f) == 1:
                    col = shear_data['f{shear_prefix}_mag_{f}{v}']
                # or a colour
                else:
                    b1,b2 = f.split('-')
                    col = shear_data['f{shear_prefix}_mag_{b1}{v}'] - shear_data['f{shear_prefix}_mag_{b2}{v}']
    ...
    def select(self, data, bin_index):
        ...
        s2n = data['f{shear_prefix}_s2n']
        T = data['f{shear_prefix}_T']
        Tpsf = data['f{shear_prefix}_psf_T_mean']
        flag = data['f{shear_prefix}_flags']
        zbin = data['zbin']
