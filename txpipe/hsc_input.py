...
class HSCInput(PipelineStage):
    """
    This stage ingests HSC data to pass it along to the mapper
    and/or two-point stages.
    """
    name='TXHSCInput'
    ...
    outputs = [
        ('shear_catalog', FitsFile),
        ('photometry_catalog', FitsFile),
    ...
    def run(self):
        ...
        bands = []
        for b in 'ugrizy':
            if f'{b}cmodel_mag' in available:
                bands.append(b)

        # Columns that we will need.
        shear_cols = ([
            'object_id', 'ira', 'idec', 'ishape_hsm_regauss_e1', 'ishape_hsm_regauss_e2', 'ishape_hsm_regauss_derived_shape_weight',
            'ishape_hsm_regauss_derived_shear_bias_m', 'ishape_hsm_regauss_derived_shear_bias_c1', 'ishape_hsm_regauss_derived_shear_bias_c2',
            'ishape_hsm_regauss_derived_sigma_e', 'ishape_hsm_regauss_derived_rms_e', 'ishape_hsm_regauss_sigma', 'ishape_hsm_regauss_resolution',
            'iblendedness_abs_flux', 'iclassification_extendedness', 'iflags_pixel_bright_object_center', 'iflags_pixel_bright_object_center_isnull',
            'iflags_pixel_bright_object_any', 'iflags_pixel_bright_object_any_isnull' 
            ]
        )

        # Input columns for photometry
        photo_cols = ['object_id', 'ira', 'idec']

        # Photometry columns (non-metacal)
        for band in 'ugrizy':
            photo_cols.append(f'{band}cmodel_mag')
            photo_cols.append(f'{band}cmodel_mag_err')
        ...
        shear_out_cols['ra'] = shear_cols['ira']
        shear_out_cols['dec'] = shear_cols['idec']
