...
from .utils.calibration_tools import (
    metacal_variants,
    band_variants,
    ParallelCalibratorMetacal,
    ParallelCalibratorNonMetacal,
)
import numpy as np
import warnings
...
class TXSourceSelector(PipelineStage):
    ...
    inputs = [
        ...
        (
            'photometry_catalog',
            HDFFile,
        ),  # this is to get the photo-z, does not necessarily need it
    ]
    ...
    def run(self):
        ...
        number_density_stats = SourceNumberDensityStats(
            nbin_source, comm=self.comm, shear_type=self.config['shear_catalog_type']
        )

        if shear_catalog_type == 'metacal':
            calibrators = [
                ParallelCalibratorMetacal(self.select, delta_gamma)
                for i in range(nbin_source)
            ]
        else:
            calibrators = [
                ParallelCalibratorNonMetacal(self.select) for i in range(nbin_source)
            ]
        ...
        for (start, end, shear_data) in iter_shear:
            ...
            tomo_bin, R, counts = self.calculate_tomography(
                pz_data, shear_data, calibrators
            )
        ...
        for v in variants:
            ...
            for f in features:
                ...
                # or a colour
                    ...
                    b1, b2 = f.split('-')
                    col = (
                        shear_data[f'{shear_prefix}mag_{b1}{v}']
                        - shear_data[f'{shear_prefix}mag_{b2}{v}']
                    )
    ...
    def select(self, data, bin_index):
        ...
        if verbose:
            print(
                f"Bin {bin_index} ({variant}) {f1:.2%} flag, {f2:.2%} size, {f3:.2%} SNR, {f4:.2%} z"
            )
