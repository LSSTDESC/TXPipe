...
class CMSelectHalosSkySim(PipelineStage):
    name = "CMSelectHalos"
    parallel = False
    inputs = [("cluster_SkySim_halo_catalog", HDFFile)]
    ...
    def run(self):
        #TO DO : find a way to convert directly at the ingestion phase
        h0_SkySim = 0.71
        ...
        log10medge = np.concatenate([[-np.inf], log10medge, [np.inf]])
        ...
        cols = ["halo_mass", "redshift", "ra", "dec", "baseDC2/sod_halo_mass", "baseDC2/sod_halo_radius", "halo_id"]


        f = self.open_output("cluster_SkySim_halo_tomography")
        ...
        it = self.iterate_hdf("cluster_SkySim_halo_catalog", "halos", cols, chunk_rows)
        ...
        for (i, j), name in zip(bins, bin_names):
            ...
            metadata['mass_min'] = 10**log10medge[i+1]
            metadata['mass_max'] = 10**log10medge[i+2]
