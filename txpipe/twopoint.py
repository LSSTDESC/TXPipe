...
class TXTwoPoint(PipelineStage):
    ...
    def get_m(self, data, i):
        ...
        if self.config['shear_catalog_type'] == 'metacal':
            g1, g2 = apply_metacal_response(
                data['R'][i], 0.0, data['mcal_g1'][mask], data['mcal_g2'][mask]
            )
            return g1, g2, mask
            ...
            raise ValueError(
                f"Please specify metacal or lensfit for shear_catalog in config."
            )
    ...
    def get_shear_catalog(self, data, i):
        ...
        if (
            self.config['var_methods'] == 'jackknife'
            and self.config['shear_catalog_type'] == 'metacal'
        ):
            ...
            self.config['var_methods'] == 'jackknife'
            and self.config['shear_catalog_type'] == 'lensfit'
        ):
            patch_centers = self.get_input('patch_centers')
            g1, g2, mask = self.get_m(data, i)
            cat = treecorr.Catalog(
    ...
    def calculate_pos_pos(self, data, i, j):
        ...
        print(
            f"Rank {self.rank} calculating position-position bin pair ({i},{j}): {n_i} x {n_j} objects, "
    ...
    def calculate_area(self, data):
        ...
        pix = hp.ang2pix(
            4096,
            np.pi / 2.0 - np.radians(data['dec']),
            np.radians(data['ra']),
            nest=True,
        )
        area = hp.nside2pixarea(4096) * (180.0 / np.pi) ** 2
        mask = np.bincount(pix) > 0
        area = np.sum(mask) * area
        area = float(area) * 60.0 * 60.0
        return area
    ...
    def run(self):
        ...
        outputs = {
            "galaxy_density_xi": self.open_output(
                'density_xi', figsize=(nbin_lens * 5, nbin_lens * 3), wrapper=True
            ),
            "galaxy_shearDensity_xi_t": self.open_output(
                'shearDensity_xi',
                figsize=(5 * nbin_lens, 3 * nbin_source),
                wrapper=True,
            ),
            "galaxy_shear_xi_plus": self.open_output(
                'shear_xi_plus',
                figsize=(5 * nbin_source, 3 * nbin_source),
                wrapper=True,
            ),
