...
class TXSourceSelectorAnacal(TXSourceSelectorBase):
    ...
    def apply_simple_redshift_cut(self, shear_data):
        """Extend the base tomographic binning to emit ±γ shifted variants
        of every quantity the AnaCal selector cuts on: zbin (from mean_z),
        s2n (from the analytic derivative ds2n_dg), and the AnaCal size
        moments m00, m20 (from dm00_dg, dm20_dg).

        Once these are in ``pz_data`` (which the base
        ``calculate_tomography`` merges into ``data`` before dispatch),
        ``AnaCalCalculator`` needs no special-case size-cut logic — the
        _DataWrapper suffix lookup routes every cut through the same code
        path uniformly.
        ...
        # m00 and m20 variants: needed for the size cut (m00+m20)/m00 > 0.1
